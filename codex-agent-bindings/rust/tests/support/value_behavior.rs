...
fn compile_header_references(headers: &BTreeSet<String>, stale: bool) -> bool {
    ...
    let result = Command::new(std::env::var_os("CC").unwrap_or_else(|| "cc".into()))
        ...
        .arg("-Werror")
        .arg("-I")
        .arg(super::artifact_inputs::c_include_directory())
        .arg("-c")
        .arg(source)
        .arg("-o")
        .arg(object)
...
fn execute_real_sdk_null_boundaries(
    ...
    let header_path = super::artifact_inputs::c_header();
    let header = super::artifact_inputs::read(&header_path, "exact public C header");
    let library = super::artifact_inputs::real_sdk_library();
    let symbols = rows
        .iter()
...
pub fn verify_and_execute(rows: &[Vec<&str>]) -> Evidence {
    ...
    let canonical_report = super::artifact_inputs::read(
        &super::artifact_inputs::canonical_api_report(),
        "canonical compiler report",
    );
    let canonical_keys = canonical_keys(&canonical_report);
    let bootstrap_report = super::artifact_inputs::read(
        &super::artifact_inputs::c_abi_bootstrap_evidence(),
        "canonical C ABI evidence",
    );
