...
#[path = "support/artifact_inputs.rs"]
mod artifact_inputs;
...
fn evidence_values() -> Vec<i32> {
    ...
    let include = artifact_inputs::c_include_directory();
...
fn ordinary_enum_claims_match_canonical_compiler_and_header_evidence() {
    let canonical_report = artifact_inputs::read(
        &artifact_inputs::canonical_api_report(),
        "compiler-derived canonical API report",
    );
