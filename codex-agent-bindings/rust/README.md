...
crate/import are `codex-agent`/`codex_agent`; before `<sdkVersion>` is
published, consume a locally built `codex-agent-<sdkVersion>.crate`. The crate
...
[`consumer/src/bin/lifecycle_smoke.rs`](https://github.com/codex-agent-labs/codex-agent/blob/main/codex-agent-bindings/rust/consumer/src/bin/lifecycle_smoke.rs).
...
and `CODEX_AGENT_C_SDK_ROOT` (which must contain `include/codex_agent.h`).
Matching-host real-SDK tests additionally use `CODEX_AGENT_REAL_SDK`;
runtime loading and installed consumers retain `CODEX_AGENT_LIBRARY`.
