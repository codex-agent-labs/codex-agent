...
`CodexAgent.<sdkVersion>.nupkg`; it is not claimed as published to NuGet.org. It does
...
dotnet build tests/CodexAgent.Tests/CodexAgent.Tests.csproj --configuration Release \
  ...
  -p:CodexAgentCAbiBootstrapEvidence=/path/to/bootstrap-evidence.json \
  -p:CodexAgentCSdkRoot=/path/to/c-sdk \
