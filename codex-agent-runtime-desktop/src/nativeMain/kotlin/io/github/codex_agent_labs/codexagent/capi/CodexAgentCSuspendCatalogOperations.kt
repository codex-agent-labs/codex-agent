...
public fun codexAgentMcpServersAdd(
    ...
    val copied = copyCatalogInput<CodexAgentCMcpServerConfigurationSnapshot, io.github.codex_agent_labs.codexagent.agent.AgentMcpServerConfiguration>(
        context,
        configuration,
...
public fun codexAgentMcpServersRemove(
    ...
    val copied = copyCatalogInput<CodexAgentCMcpServerSnapshot, io.github.codex_agent_labs.codexagent.agent.AgentMcpServer>(
        context,
        server,
    ) { it.value.cAbiOwnedCopy() }
    if (copied.status != CODEX_AGENT_STATUS_OK) return@abiStatus copied.status
