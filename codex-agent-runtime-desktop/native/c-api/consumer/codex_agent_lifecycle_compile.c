...
_Static_assert(CODEX_AGENT_ABI_VERSION_MINOR == UINT32_C(5), "ABI minor");
_Static_assert(CODEX_AGENT_ABI_VERSION_PATCH == UINT32_C(0), "ABI patch");
_Static_assert(
    CODEX_AGENT_ABI_VERSION_CURRENT == UINT32_C(0x01050000),
    "ABI current");
_Static_assert(
...
int main(void) {
    ...
    CHECK(
        codex_agent_abi_is_compatible(CODEX_AGENT_ABI_VERSION_ENCODE(1, 6, 0)) ==
        INT32_C(0));
