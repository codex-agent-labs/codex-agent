...
/* Every value is compiled from the exact public C SDK header. */
...
int main(void) {
    const size_t count = sizeof(canonical_enum_values) / sizeof(canonical_enum_values[0]);
    for (size_t index = 0; index < count; ++index) {
        if (printf("%" PRId32 "\n", canonical_enum_values[index]) < 0) return 1;
    }
    return 0;
}
