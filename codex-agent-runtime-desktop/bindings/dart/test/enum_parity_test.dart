...
Set<String> _canonicalEnumCapabilities(Directory repositoryRoot) {
  final report = File(
    '${repositoryRoot.path}/codex-agent-core/build/reports/'
    'cross-language-api/canonical-api.json',
  );
  if (!report.existsSync()) {
    throw StateError('canonical API report is required: ${report.path}');
  }
  final document =
...
Map<String, dynamic> _bootstrap(Directory repositoryRoot) => jsonDecode(
      File(
        '${repositoryRoot.path}/codex-agent-runtime-desktop/build/reports/'
...
Set<String> _verifyValueClaims(
  ...
  final header = File(
    '${repositoryRoot.path}/codex-agent-runtime-desktop/native/c-api/include/'
    'codex_agent.h',
  ).readAsStringSync();
