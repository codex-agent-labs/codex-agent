...
`codex-agent-core/protocol/schema/provenance.json`. The checked-in stable-v2
schema digest is
...
`./gradlew :codex-agent-core:verifyProtocolSource` verifies those inputs and
all generated outputs. To regenerate, check out the recorded Codex revision and
run `:codex-agent-core:updateProtocol` with the four `codexProtocol*` file
